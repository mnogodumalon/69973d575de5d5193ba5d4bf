...
import { ArtikelEinstellenViewDialog } from '@/components/dialogs/ArtikelEinstellenViewDialog';
...
const ARTIKELEINSTELLEN_FIELDS = [
  ...
  { key: 'groesse', label: 'Größe', type: 'string/text' },
];

const ENTITY_TABS = [
  { key: 'artikel_einstellen', label: 'Artikel einstellen', pascal: 'ArtikelEinstellen' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('artikel_einstellen');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    artikel_einstellen: new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'artikel_einstellen': return (data as any).artikelEinstellen as ArtikelEinstellen[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'artikel_einstellen': return ARTIKELEINSTELLEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'artikel_einstellen': return {
        create: (fields: any) => LivingAppsService.createArtikelEinstellenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateArtikelEinstellenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteArtikelEinstellenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'artikel_einstellen' || dialogState?.entity === 'artikel_einstellen') && (
        <ArtikelEinstellenDialog
          open={createEntity === 'artikel_einstellen' || dialogState?.entity === 'artikel_einstellen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'artikel_einstellen' ? handleUpdate : (fields: any) => handleCreate('artikel_einstellen', fields)}
          defaultValues={dialogState?.entity === 'artikel_einstellen' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['ArtikelEinstellen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['ArtikelEinstellen']}
        />
      )}
      {viewState?.entity === 'artikel_einstellen' && (
        <ArtikelEinstellenViewDialog
          open={viewState?.entity === 'artikel_einstellen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'artikel_einstellen', record: r }); }}
        />
      )}
