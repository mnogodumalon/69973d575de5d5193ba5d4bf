...
export class LivingAppsService {
  ...
  static async getArtikelEinstellen(): Promise<ArtikelEinstellen[]> {
    ...
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as ArtikelEinstellen[];
    return enrichLookupFields(records, 'artikel_einstellen');
  }
  static async getArtikelEinstellenEntry(id: string): Promise<ArtikelEinstellen | undefined> {
    ...
    const record = { record_id: data.id, ...data } as ArtikelEinstellen;
    return enrichLookupFields([record], 'artikel_einstellen')[0];
  }
